...
# Set page config
st.set_page_config(
    page_title="Governance | AI Governance Dashboard",
    page_icon="🛡️",
    layout="wide"
)
...
tab1, tab2, tab3, tab4 = st.tabs(["Policies", "Create Policy", "Generate Policy", "Recommendations"])
...
with tab1:
    ...
    if filtered_policies:
        policy_df = pd.DataFrame([
            {
                ...
                "Status": p.get("status"),
                "Created": p.get("created_at").split("T")[0] if p.get("created_at") else "Unknown",
            }
        ...
        selected_policy_id = st.selectbox(
            ...
            format_func=lambda x: next((p.get("title") for p in filtered_policies if p.get("id") == x), "Unknown")
        )
        
        # Display selected policy
        selected_policy = next((p for p in filtered_policies if p.get("id") == selected_policy_id), None)
        if selected_policy:
            ...
            with col3:
                st.markdown(f"**Last Updated:** {selected_policy.get('updated_at').split('T')[0] if selected_policy.get('updated_at') else 'Unknown'}")
            
            st.markdown("### Description")
            st.markdown(selected_policy.get("description", "No description available"))
            ...
            if hasattr(st.session_state, "edit_policy_id") and st.session_state.edit_policy_id == selected_policy_id:
                st.subheader("Edit Policy")
                edit_title = st.text_input("Title", value=selected_policy.get("title", ""))
                edit_category = st.selectbox(
                    "Category",
                    options=categories,
                    index=categories.index(selected_policy.get("category")) if selected_policy.get("category") in categories else 0
                )
                edit_status = st.selectbox(
                    "Status",
                    options=["Draft", "Active", "Under Review", "Deprecated"],
                    index=["Draft", "Active", "Under Review", "Deprecated"].index(selected_policy.get("status")) if selected_policy.get("status") in ["Draft", "Active", "Under Review", "Deprecated"] else 0
                )
                edit_description = st.text_area("Description", value=selected_policy.get("description", ""), height=100)
                edit_content = st.text_area("Content", value=selected_policy.get("content", ""), height=300)
                    ...
                    if st.button("Save Changes"):
                        try:
                            ...
                            if response.status_code == 200:
                                ...
                                if hasattr(st.session_state, "edit_policy_data"):
...
with tab2:
    ...
    with st.form("create_policy_form"):
        ...
        new_status = st.selectbox("Status", options=["Draft", "Active", "Under Review"])
...
with tab3:
    ...
    st.markdown("Let the AI agent generate a governance policy for you based on a selected category.")
        ...
        with st.spinner("Generating policy... This may take a moment."):
            try:
                ...
                if response.status_code == 200:
                    ...
                    st.markdown("### Description")
                    st.markdown(policy.get("description", "No description available"))
...
with tab4:
    ...
    st.markdown("The AI governance agent analyzes your existing policies and identifies gaps and improvement opportunities.")
    
    # Display recommendations
    if recommendations:
        for i, rec in enumerate(recommendations):
            with st.expander(f"{rec.get('title', 'Policy Recommendation')} ({rec.get('priority', 'Medium')})"):
        ...
        st.info("No policy recommendations found. This may indicate that your governance framework is already comprehensive!")
