...
function renderDashboardCharts() {
  const complianceChart = document.getElementById('compliance-status-chart');
  const riskChart = document.getElementById('risk-distribution-chart');

  if (complianceChart) {
    fetch('/api/dashboard/compliance-status')
      .then(response => response.json())
      .then(data => initComplianceStatusChart(complianceChart, data))
      .catch(error => console.error('Error loading compliance chart:', error));
  }

  if (riskChart) {
    fetch('/api/dashboard/risk-distribution')
      .then(response => response.json())
      .then(data => initRiskDistributionChart(riskChart, data))
      .catch(error => console.error('Error loading risk chart:', error));
  }
}

export function initComplianceStatusChart(element, data) {
  const trace = {
    values: data.values,
    labels: data.labels,
    type: 'pie',
...
export function initRiskDistributionChart(element, data) {
  const trace = {
    x: data.categories,
    y: data.values,
    type: 'bar',
